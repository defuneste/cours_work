# Date: March 2022
# Olivier
# Homework week 8 Statictical Rethinking
# https://github.com/rmcelreath/stat_rethinking_2022/blob/main/homework/week08.pdf
# I Just reproduce solutions !
...
f_dyad <- alist(
  ...
  logit(pBA) <- a + T[D,2],
  a ~ normal(0,1),
# lost here dyads - non centered
  transpars> matrix[N_dyads, 2]:T <- compose_noncentered(rep_vector(sigma_T, 2),
                                                         L_Rho_T,
                                                         Z),
# rest is fine
  ...
  ## correlation matrix for dyads, still this <<-
  gq> matrix[2,2]:Rho_T <<-  Chol_to_Corr( L_Rho_T )
)

m1 <- ulam(f_dyad,
           data = monk_dat, chains = 4, cores = 4, iter = 2000) # more iter here also

precis(m1,depth = 3, pars = c("Rho_T","sigma_T")) # pars est cool !

# 0.63 +/- 0.11 hard to know if it is big or not
...
m2 <- ulam( f_dyad2 ,
           data=monk_dat , chains=4 , cores=4 , iter=2000 )

precis( m2 , depth=3 , pars=c("Rho_T","Rho_T2") )

# reciprocity in liking is stronguer but dislike is also shared

post <- extract.samples(m2)
round(quantile( post$Rho_T[,1,2] - post$Rho_T2[,1,2] ),2)

dens(post$Rho_T[,1,2], col = 2)
dens(post$Rho_T2[,1,2], col = 4, add = TRUE)i
...
plot(sng , layout=lx , vertex.size=8 , edge.arrow.size=1 ,
edge.width=2 , edge.curved=0.35 , vertex.color=2 , edge.color=grau() ,
asp=0.9 , margin = -0.05 , vertex.label=NA )

# dislike between clusters, more targeted
sng2 <- graph_from_adjacency_matrix(y2_est)
plot(sng2 , layout=lx , vertex.size=8 , edge.arrow.size=1 ,
edge.width=2 , edge.curved=0.35 , vertex.color=6 , edge.color=grau() ,
asp=0.9 , margin = -0.05 , vertex.label=NA)
...
# Rho_2 is low but with hight variation. sigma_r2 is important and seems to be drived with fews monks really disliked while other doesnt seems to receive any kind of "negativity"
...
# la version colorée est bcp plus classe
